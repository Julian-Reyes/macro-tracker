...
│       ├── QuickAddSection.jsx # Favorites + recent meals for quick re-log
...
│       │   ├── meals.js  # POST /, POST /scan, POST /import, GET /, GET /history/range, GET /favorites, GET /recent, GET /:id, PATCH /:id/favorite, PUT /:id, DELETE /:id
...
- `GET /api/meals?date=2026-03-20&tz=240` — list meals for a day with totals. `tz` param = client's `getTimezoneOffset()` (minutes) to align server query window with the client's local day
- `GET /api/meals/history/range?from=...&to=...&tz=240` — date range query. Same `tz` offset for correct day boundaries
- `GET /api/meals/:id` — single meal with items
- `GET /api/meals/favorites` — **auth required** — all favorited meals with items, ordered by most recent
- `GET /api/meals/recent?days=14` — **auth required** — deduplicated recent meals (by sorted item name fingerprint), limit 20
- `PATCH /api/meals/:id/favorite` — **auth required** — `{ isFavorite: true|false }` → toggle favorite status
...
### What works
- Frontend: guest mode (try before sign up), login/register screens (shown as overlay), camera/gallery capture with optional meal description text input for improved AI accuracy, base64 encoding + client-side downscaling, AI analysis via backend API, results display with macro rings and item breakdown, persistent daily log (localStorage for guests, DB for authenticated), delete meals with undo toast (4-second window to undo before actual deletion), edit saved meals (tap meal in log → detail view → "Edit Meal" → adjust portions/items/type → "Save Changes" updates in place via PUT endpoint), goals-based MacroRing max values, guest data migration to DB on registration, logo click navigates to fresh scan, scan state resets after adding meal to log (including meal description text), date-based daily view with prev/next navigation (meals and totals filtered per day for both guest and authenticated modes), meal detail view (click any logged meal to see full macro breakdown, image, item list, and notes), guest meal images persisted as downscaled data URLs in localStorage, authenticated meal images resolved via `resolveImageUrl()` (prepends backend origin to relative `/uploads/...` paths for split-hosting compatibility), portion editing via per-item multiplier (tap item → ×0.25 step stepper + optional gram input for items with gram-based portions, macro rings and totals update in real-time, portion description text scales dynamically with multiplier, adjusted values saved to log, multiplier persisted in DB and restored on re-edit), manual food entry via text search (USDA/OFF lookup with per-100g scaling, debounced search, gram-based portion input), meal type labels (Breakfast/Lunch/Dinner/Snack) with time-based auto-selection and grouped daily log, weekly summary view with interactive SVG bar chart (metric switching between calories/protein/carbs/fat, week navigation, tap bar to jump to daily view), add extra items to scanned meals (inline USDA/OFF search in result view, combined with AI-analyzed items before saving), remaining macros display on daily view ("# MACRO Remaining" format, shown only on today when meals are logged), unified ItemRow component for all food items (AI-analyzed, search-added extras, manual entries) with consistent macro bars and removable via ✕ on expand, PWA installable (manifest + service worker + Apple meta tags), goals editing UI (profile-based auto-calculation: height/weight/age/sex/activity/goal → Mifflin-St Jeor BMR → TDEE → macro split, with imperial/metric toggle and live preview), bottom tab bar navigation (Scan/Log/Stats/Goals with SVG icons, iOS safe area support, hidden during sub-views), barcode scanner for packaged foods (live camera via html5-qrcode, photo fallback via BarcodeDetector API + html5-qrcode scanFile, manual code entry — results feed into manual entry flow for portion adjustment), refactored view layer (CaptureView, ManualEntryView, BarcodeScannerView extracted from App.jsx), favorite meals (star toggle in daily log, `isFavorite` on Meal model) with quick re-log from Scan tab, recent meals (deduplicated last 14 days) shown on Scan tab for quick re-logging
- Backend: all routes working, SQLite via Prisma, AI multi-provider dispatcher, USDA + Open Food Facts lookup with DB caching (USDA searches Foundation + SR Legacy + FNDDS data types for broad coverage; OFF results filtered for English language, valid nutrition data, and rounded values), barcode lookup via OFF barcode API (cached in NutritionCache), JWT auth (crashes if JWT_SECRET missing), rate limiting on analyze + scan + search + barcode endpoints (with `trust proxy` for correct IP detection behind Fly.io), serves static React build in production, guest analyze endpoint (no auth), bulk import endpoint for guest data migration, sanitized error messages (generic errors to client, full details server-side only), timezone-aware date queries (client sends `tz` offset, server adjusts UTC window)
- Deployment: Live — frontend auto-deploys to GitHub Pages on push, backend on Fly.io (manual `fly deploy`). Fly.io volume mounts /data for SQLite + uploads. Dockerfile runs `prisma db push` on startup to ensure tables exist
...
- **Guest favorites**: `isFavorite` field on localStorage meals, toggled via `toggleGuestMealFavorite(localId)`. Each guest meal gets a stable `localId` (timestamp+random) on creation; older meals without one get backfilled on first `getGuestMeals()` call
...
- **Guest Goals**: Guests can fully use the Goals page — enter profile data, see calculated macros, and save to localStorage. On registration, guest goals migrate to DB via `PUT /api/goals/profile`. `macro_guest_goals` localStorage key stores `{ profile, goals }`
...
- **Helmet**: Security headers via `helmet` middleware. `crossOriginResourcePolicy` set to `cross-origin` and CSP `img-src` includes `CORS_ORIGIN` to allow cross-origin image loading (split hosting: frontend on GitHub Pages loads images from Fly.io backend)
- **Trust proxy**: `app.set('trust proxy', 1)` in production — required for `express-rate-limit` to work correctly behind Fly.io's reverse proxy
...
- Prisma field names use camelCase (proteinG) but the AI returns snake_case (protein_g) — the meals route maps between them when saving, and `normalizeItem()` maps back for display (also carries `multiplier` field, defaulting to 1.0)
...
- Timezone handling: meal date queries require the client's `tz` offset (via `getTimezoneOffset()`) so the server constructs the correct local-day window in UTC. Without this, meals logged late evening in western timezones appear on the wrong day. The `tz` param is sent on `getMeals()` and `getMealsRange()` API calls. Guest mode is unaffected (filters client-side via `toLocalDateStr()`)
- Authenticated meal images: stored on disk as `/uploads/filename.jpg` (relative path in DB). Due to split hosting (frontend on GitHub Pages, backend on Fly.io), these must be resolved via `resolveImageUrl()` before rendering in `<img>` tags. Guest images are data URLs and need no resolution
- Service worker caches aggressively — when deploying breaking frontend changes, bump `CACHE_NAME` in `public/sw.js` (e.g., `macro-v1` → `macro-v2`) so old caches are purged on activate. Vite's hashed filenames help (new JS bundles get new URLs), but the SW itself and index.html are not hashed. In dev mode, the SW is not registered and existing SWs are unregistered (see main.jsx)
...
- [x] Favorite/recent meals — star meals in daily log, quick re-log from Scan tab (favorites + deduplicated recents)
- [x] Dynamic portion descriptions — portion text scales numbers in real-time with multiplier
- [x] Manual gram input — type exact gram amount in expanded item row, auto-calculates multiplier from base gram value in portion string
- [x] Cross-origin image loading — resolveImageUrl() for split-hosting, Helmet CORP/CSP configured
- [x] Timezone-aware date queries — client sends tz offset so server constructs correct local-day window
- [x] Dockerfile runs prisma db push on startup — ensures DB tables exist on first deploy
- [x] Trust proxy + CORS PATCH — correct rate limiting behind Fly.io proxy, PATCH method for favorite toggle
...
5. ~~**Favorite/recent meals**~~ ✅ Star meals in daily log (`isFavorite` on Meal model, `PATCH /:id/favorite`). Scan tab shows "Favorites" (horizontal scroll cards) + "Recent" (deduplicated last 14 days, vertical rows). Tap any to re-log: loads into result view for portion adjustment, then "Add to Log" saves as new meal. Guest favorites use `localId` on localStorage meals. `QuickAddSection` component renders both sections. Recents deduplicated by sorted item-name fingerprint
...
9. ~~**Meal editing** — Adjust portion sizes or macros after AI analysis~~ ✅ Per-item multiplier on scan results + edit saved meals (tap meal in daily log → "Edit Meal" → adjust portions, add/remove items, change meal type → "Save Changes" updates via `PUT /api/meals/:id`). Multiplier persisted in `MealItem.multiplier` column — re-editing restores the saved multiplier and reconstructs base values. Guest edits update localStorage by index via `updateGuestMeal()`
...
- API client centralizes all fetch calls, token management, 401 handling, guest localStorage helpers, and `resolveImageUrl()` in src/api.js. `resolveImageUrl()` prepends the backend origin (`UPLOADS_BASE`) to relative `/uploads/...` paths — required because split hosting means the frontend domain differs from the backend. Passes through data URLs (guest images) and absolute URLs unchanged. Used in DailyLogView, QuickAddSection, and App.jsx anywhere `imageUrl` is rendered or set as `image` state
- Scan flow: both guest and authenticated use `analyzeMeal()` (analyze only) → user edits portions → optionally add extra items via inline search (`extraItems` state, `addExtraItemToScan()`) → save. Authenticated saves via `saveMeal()` → `POST /api/meals`. Guest saves via `addGuestMeal()` to localStorage. `adjustedItems` useMemo combines scanned items (with multipliers) + extra items (with multipliers) for totals and saving — each item in the output includes a `multiplier` field that gets persisted to the DB/localStorage. AI items removable via `removeAnalysisItem()`, extra items via `removeExtraItem()`
...
- ItemRow component: unified display for all food items (AI-analyzed, extra search items, manual entries). Shows name, portion, calories, P/C/F macro bars. Portion description text scales dynamically with multiplier (all numbers in the string are multiplied). Tap to expand: AI items and extra items show `PortionControls` (×multiplier −/+ buttons + gram input when portion contains gram values) + ✕ remove, manual items show ✕ only (`onMultiplierChange={null}` hides controls). `extractBaseGrams()` parses gram values from portion strings to enable direct gram entry — typing a gram amount auto-calculates the multiplier as `inputGrams / baseGrams`
- Edit saved meals flow: tap meal in daily log → `mealDetailMode=true` + `editingMealId` set (meal ID for auth, array index for guest) → read-only detail view with "Back to Log" and "Edit Meal" buttons → `handleStartEdit()` snapshots `originalAnalysis`, restores saved multipliers into `itemAdjustments`, and divides stored macro values by multiplier to reconstruct base values → all editing controls enabled (multipliers, add/remove items, meal type picker) → "Save Changes" calls `updateMeal()` (auth) or `updateGuestMeal()` (guest) with `adjustedItems` (which include `multiplier` on each item) → "Cancel" reverts to `originalAnalysis`. The `useEffect` on `analysis` is guarded by `isEditingMeal` to prevent resetting adjustments during editing
- Meal description: optional `mealDescription` state, shown as text input in CaptureView after image preview (before Retake/Analyze). Passed through `analyzeMeal()` → `POST /api/meals/analyze` → `analyzeFood()` → AI provider. Appended to user prompt as "The user describes this meal as: ..." when non-empty. Cleared in `resetCapture()` and `addToDaily()`
- Capture button order: Gallery (outline) → Camera (gold, center focus) → Barcode (outline) — Camera centered as primary action
- Guest localStorage helpers: `getGuestMeals()`/`getGuestMealsByDate()`/`addGuestMeal()`/`deleteGuestMeal()`/`updateGuestMeal()`/`toggleGuestMealFavorite()`/`getGuestFavoriteMeals()`/`getGuestRecentMeals()`/`clearGuestMeals()`, `importMeals()` for DB migration. Each guest meal gets a `localId` for stable identification (backfilled on first read for older meals). Goals: `getGuestGoals()`/`saveGuestGoals()`/`clearGuestGoals()` — stores `{ profile, goals }` in `macro_guest_goals` localStorage key
...
- PWA: manifest.json + sw.js live in `public/` (copied to dist root by Vite). Service worker registered in main.jsx using `import.meta.env.BASE_URL + 'sw.js'` — **production only** (`import.meta.env.PROD`). In dev mode, any existing SW is unregistered to prevent stale cache issues during testing. Cache version bumped via `CACHE_NAME` in sw.js. Apple PWA meta tags in index.html for iOS standalone mode. `viewport-fit=cover` enables `env(safe-area-inset-bottom)` for home indicator padding
- Navigation: `BottomTabBar` component with 4 tabs (Scan/Log/Stats/Goals) using inline SVG icons. Fixed to bottom with `env(safe-area-inset-bottom)` padding. Active tab: gold `#E8C872`, inactive: `rgba(255,255,255,0.45)`. Hidden during `result` and `manual` sub-views. Tab bar renders conditionally in App.jsx based on `view` state. Goals tab fully functional for guests (localStorage). Header simplified to logo + "Login" button (guests) / "Log out" button (authenticated)
...
- Favorite/recent meals: `QuickAddSection` component renders on CaptureView (Scan tab) when `!image`. Favorites shown as horizontal scroll cards, recents as vertical compact rows. Recents deduplicated by fingerprint (`items.map(i => i.name.trim().toLowerCase()).sort().join('|')`), excluding meals already in favorites. Star toggle on daily log meal rows (between time and delete button) with optimistic UI update. `handleRelogMeal()` loads a historical meal into the result view as a new meal (`mealDetailMode=false`), reusing the existing `addToDaily()` flow. `loadQuickAddData()` fetches both lists when switching to capture view
- Re-log flow: tap favorite/recent → `handleRelogMeal()` normalizes items, computes totals, sets `analysis` + `image` → `setView("result")` with `mealDetailMode=false` → user adjusts portions → "Add to Log" saves as new meal for today. `scaledImageData` is `null` (no image re-upload for re-logged meals)
